...
fn bench_online_bayesian(c: &mut Criterion) {
    ...
    let data: Vec<f64> = raw_data.lines().skip(1).map(|line| {
        line.split_at(10).1.parse().unwrap()
    }).collect();

    c.bench("online bayesian",
        ParameterizedBenchmark::new(
            "process",
            move |b, nelems| b.iter(|| {
                ...
                let _res: Vec<Vec<f64>> = data.clone().iter().take(*nelems).map(|d| cpd.step(d)).collect();
            }), (0..500).step_by(100)
        )
